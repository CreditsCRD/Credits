...
static const char *strDNSSeed[][2] = {
    {"mint.mine.bz", "mint.mine.bz"},
    {"minter.ftp.sh", "minter.ftp.sh"},
    {"minez.allowed.org", "minez.allowed.org"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Credits is probably already running."), addrBind.ToString().c_str());
        else
